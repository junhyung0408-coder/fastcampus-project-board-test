...
def _latest_two_closes(symbol: str, target_day: date) -> tuple[float, float]:
    """
...
def build_snapshot() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for name, symbol in index_symbols.items():
        current, previous = _latest_two_closes(symbol, report_day)
    ...
    return pd.DataFrame(index_rows), pd.DataFrame(fx_rows)


def render_html(index_df: pd.DataFrame, fx_df: pd.DataFrame, report_day: date) -> str:
    index_html = index_df.to_html(index=False, border=0, classes="tbl")
    fx_html = fx_df.to_html(index=False, border=0, classes="tbl")
...
def save_report() -> Path:
    report_day = previous_business_day()
    index_df, fx_df = build_snapshot()
    html = render_html(index_df, fx_df, report_day)
